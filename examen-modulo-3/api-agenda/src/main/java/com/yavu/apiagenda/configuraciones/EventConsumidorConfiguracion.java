...
public class EventConsumidorConfiguracion implements RabbitListenerConfigurer {
	...
	public Queue queueRollback() {
		return new Queue("rollbackAgendar");
	}

	@Bean
	public Binding bindingRollback(Queue queueRollback, TopicExchange topicExchange) {
		return BindingBuilder.bind(queueRollback).to(topicExchange).with("rollback.agendar");
	}
	...
	public Binding binding(Queue queue, DirectExchange eventExchange) {
		return BindingBuilder.bind(queue).to(eventExchange).with("saga.agendar");
	}

	@Bean
	public EventReceiverMessageConsumidor eventSenderMessageConsumidor() {
